...
> 已经很久没有用过request了，有点生疏，回顾一下感觉非常棒，还是需要多加练习，温故而知新。
...
>8.1 Scrapy架构官方文档介绍: https://docs.scrapy.org/en/latest/topics/architecture.html

>8.2 Scrapy流程图

> > <img src="https://static001.geekbang.org/resource/image/e2/21/e2954a386344f0c1a065b4e0bc384a21.png" width="90%">
...
> > 6.数据下载成功后会被封装到response中,随后response会被交给下载器,下载器将response交给引擎

> > 7.引擎将response交给spiders

> > 8.spiders拿到response后调用回调方法进行数据解析,解析成功后生成item,随后spiders将item交给引擎拿到item后进行数据的持久化存储

>8.3 Scrapy核心组件

> >  `引擎(Scrapy)` 对整个系统的数据流进行处理, 触发事务(框架核心).

> > `调度器(Scheduler` 用来接受引擎发过来的请求. 由过滤器过滤重复的url并将其压入队列中, 在引擎再次请求的时候返回. 可以想像成一个URL（抓取网页的网址或者说是链接）的优先队列, 由它来决定下一个要抓取的网址是什么.
